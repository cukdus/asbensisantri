...
class DataPetugas extends BaseController
{
    ...
    public function updatePetugas()
    {
        ...
        if ($petugasLama['username'] != $this->request->getVar('username')) {
            $this->petugasValidationRules['username']['rules'] = 'required|min_length[6]|is_unique[users.username]';
        } else {
            // If username is not changed, remove is_unique validation
            $this->petugasValidationRules['username']['rules'] = 'required|min_length[6]';
        }
        ...
        if ($petugasLama['email'] != $this->request->getVar('email')) {
            ...
            $this->petugasValidationRules['email']['rules'] = 'required';
        }
